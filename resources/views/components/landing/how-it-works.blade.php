            ...
            @foreach ($steps as $index => $step)
                <li class="reveal group glass-panel rounded-2xl p-5 transition duration-300 hover:-translate-y-1 hover:border-primary/30 hover:shadow-[0_18px_50px_rgb(26_26_27_/_0.12)]" style="--reveal-delay: {{ ($index + 1) * 80 }}ms">
                    <div class="flex items-center justify-between gap-3">
                        ...
                        <span class="font-serif text-2xl font-bold tabular-nums text-primary/25 transition group-hover:text-primary/45">{{ str_pad((string) ($index + 1), 2, '0', STR_PAD_LEFT) }}</span>
                    </div>
                    <h3 class="mt-5 font-serif text-xl font-semibold text-secondary">{{ $step['title'] }}</h3>
